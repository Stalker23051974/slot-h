                               ...
                               $use_script_name = false)
{
    ...
    $run_count = dfCount($runs);
    $wts_count = dfCount($wts);
    ...
    $run_count = $run_count - dfCount($bad_runs);
...
function xhprof_param_init($params)
{
    ...
    foreach ($params as $k => $v) {
        ...
        if ($k === 'run') {
            $p = implode(',', array_filter(dfExplode(',', $p), 'ctype_xdigit'));
        }
